...
def agasc( ra, dec, radius=1.5, date=DateTime(), pm_correct=True):
    ...
    agasc_lines, denv = Ska.Shell.bash_shell("/proj/sot/ska/bin/mp_get_agasc.pl -ra %s -dec %s -radius %s" 
                                             % ( ra, dec, radius ))
    ...
    ra_corr[has_ra_pm] = table[has_ra_pm].RA + ( table[has_ra_pm].PM_RA
                                                 * ( dyear / milliarcsecs_per_degree ))

    dec_corr = table.DEC.copy()
    has_dec_pm = table['PM_DEC'] != -9999
    dec_corr[has_dec_pm] = table[has_dec_pm].DEC + ( table[has_dec_pm].PM_DEC
                                                 * ( dyear / milliarcsecs_per_degree ))
